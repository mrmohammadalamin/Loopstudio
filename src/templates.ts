...
function fillDefaults(layer: any): Layer {
  ...
  if (base.type === 'text') {
    base.fontStyle = base.fontStyle || 'normal';
    base.textDecoration = base.textDecoration || 'none';
    base.textAlign = base.textAlign || 'left';
    base.lineHeight = base.lineHeight || 1.2;
    base.letterSpacing = base.letterSpacing || 0;
    base.fontWeight = base.fontWeight || 'normal';
  } else if (base.type === 'image') {
    base.brightness = base.brightness ?? 100;
    base.contrast = base.contrast ?? 100;
    base.saturation = base.saturation ?? 100;
    base.blur = base.blur ?? 0;
    base.grayscale = base.grayscale ?? 0;
    base.sepia = base.sepia ?? 0;
    base.hueRotate = base.hueRotate ?? 0;
  } else if (base.type === 'shape') {
    base.fill = base.fill || '#000000';
    base.shapeType = base.shapeType || 'rectangle';
    if (base.stroke === true) base.stroke = base.strokeColor || '#000000';
    if (base.stroke === false) base.stroke = 'none';
  }
...
function generateSvgThumbnail(width: number, height: number, bgColor: string, layers: Layer[]): string {
  ...
  for (const layer of sortedLayers) {
    ...
    if (layer.type === 'shape') {
      ...
      let fill = l.fill || '#cccccc';
      if (l.fillType === 'linear-gradient' && l.gradientColors && l.gradientColors[0]) {
        fill = l.gradientColors[0];
      }
...
export const TEMPLATES = [
  {
    id: 'event-badge-1',
    name: 'Event Attendee Badge',
    category: 'Print',
    isPremium: false,
    thumbnail: 'https://images.unsplash.com/photo-1544531586-fde5298cdd40?w=300&q=80',
    width: 600,
    height: 900,
    layers: [
      {
  ...
  },
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'image', name: 'Gradient Background', x: 0, y: 0, width: 1050, height: 600, src: 'https://images.unsplash.com/photo-1557682250-33bd709cbe85?auto=format&fit=crop&w=1050&q=80', locked: false },
  ...
  // ----------------------------------------------------
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'shape', name: 'Border', x: 50, y: 50, width: 1100, height: 1500, shapeType: 'rectangle', fill: 'transparent', stroke: '#d4af37', strokeColor: '#d4af37', strokeWidth: 4 },
  ...
  },
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'shape', name: 'Outline', x: 20, y: 20, width: 560, height: 860, shapeType: 'rectangle', fill: 'transparent', stroke: '#d4af37', strokeColor: '#52525b', strokeWidth: 4 },
  ...
  },
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'image', name: 'Vibrant BG', x: 0, y: 0, width: 600, height: 900, src: 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&w=600&q=80', locked: false, opacity: 0.6 },
  ...
  // ----------------------------------------------------
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'shape', name: 'Center Square', x: 90, y: 90, width: 900, height: 900, shapeType: 'rectangle', fill: 'transparent', stroke: '#d4af37', strokeColor: '#ffffff', strokeWidth: 8 },
  ...
  },
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'image', name: 'House Photo', x: 0, y: 0, width: 1080, height: 700, src: 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&w=1080&q=80', locked: false },
  ...
  },
  {
    ...
    layers: [
      { id: crypto.randomUUID(), type: 'image', name: 'Product BG', x: 960, y: 0, width: 960, height: 600, src: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=960&q=80', locked: false },
...
TEMPLATES.forEach(tmpl => {
  tmpl.thumbnail = generateSvgThumbnail(tmpl.width, tmpl.height, tmpl.backgroundColor || '#ffffff', tmpl.layers);
});
